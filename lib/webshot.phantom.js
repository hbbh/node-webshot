...
var toOverwrite = optUtils.mergeObjects(
  optUtils.filterObject(options, optUtils.phantomPage), page);
...
var _takeScreenshot = function(status) {
  ...
  window.setTimeout(function() {
    ...
    if (options.localStorage) {
      console.log("localStorage", options.localStorage);

      var keys = Object.keys(options.localStorage);
...
if (options.onCallback && options.takeShotOnCallback) {
  takeScreenshot = function(data) {
    buildEvaluationFn(
      options.onCallback.fn, options.onCallback.context)(data);
  ...
  takeScreenshot = function(status) {
    buildEvaluationFn(
      options.onLoadFinished.fn, options.onLoadFinished.context)(status);
...
function pixelCount(page, dimension, value) {
  ...
  var pageDimensions = page.evaluate(function(zoomFactor) {
    ...
    return {
      width: Math.max(
        body.offsetWidth, body.scrollWidth, documentElement.clientWidth, documentElement.scrollWidth, documentElement.offsetWidth
      ) * zoomFactor,
      height: Math.max(
        body.offsetHeight, body.scrollHeight, documentElement.clientHeight, documentElement.scrollHeight, documentElement.offsetHeight
      ) * zoomFactor
    };
